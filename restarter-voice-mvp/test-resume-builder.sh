...
echo ""
echo "🔍 測試GPT API..."
if curl -s -X POST http://localhost:3001/api/gpt -H "Content-Type: application/json" -H "X-Test-Mode: true" -d '{"userId":"test-user","messages":[{"sender":"user","text":"test"}],"system_prompt":"test"}' > /dev/null; then
    echo "✅ GPT API - 可訪問"
else
    echo "❌ GPT API - 無法訪問"
fi
...
echo ""
echo "🔧 故障排除:"
echo "- 如果生成簡歷失敗，可能是OpenAI API密鑰問題"
echo "- 檢查.env文件中的OPENAI_API_KEY設置"
echo "- 確保後端服務器正常運行"
echo ""
echo "🎉 測試完成！"
